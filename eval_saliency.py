...
from test import test, test_sal


"""
Evaluation and Visualization code for saliency map of Action Q-Transformer
"""
...
# AQT - Saliency map
# python eval_saliency.py --id save_folder_name --game game_name --evaluation-episodes 1 --architecture aqt --load-model model_path --cuda-device cuda:0
...
# Config eval
parser.add_argument('--load-model', type=str, default='results/rainbow/models/model.pth', help='Load model path')
...
# make directry
results_dir = os.path.join('visuals_sal', args.id)
...
def get_env_meta(env_name):
    ...
    if "pong" in env_name:
        ...
        meta['critic_ff'] = 600 ; meta['actor_ff'] = 200
    elif "space_invaders" in env_name:
        meta['critic_ff'] = 400 ; meta['actor_ff'] = 400
    elif "ms_pacman" in env_name:
        meta['critic_ff'] = 500 ; meta['actor_ff'] = 300
    elif "seaquest" in env_name:
        meta['critic_ff'] = 500 ; meta['actor_ff'] = 800
    else:
...
# Agent
dqn = Agent(args, env)
dqn.model_load(args.load_model)

# setting parameter for saliency
density = 12
radius = 5
meta = get_env_meta(args.game) 


print("Evaluating...")
dqn.eval()  # Set network to evaluation mode
avg_reward = test_sal(args, dqn, results_dir, density, radius, meta)  # Test
