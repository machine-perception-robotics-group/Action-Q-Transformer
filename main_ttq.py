...
from agent_ttq import Agent_ttq
from env import Env
from memory import ReplayMemory
from test import test
from utils import TTQ_REWARD_LOGGER

from glob import glob


"""
Train code for Action Q-Transformer + Target Trained Q-Network
"""


# Example code

## AQT with TTQ (alpha is linearly decaying)
# python main_ttq.py --id **_rainbow_aqt_ttq --game ** --architecture aqt --T-max 50000000 --evaluation-interval 100000 --save-interval 500000 --memory --ttq-target-model results/**_rainbow/models/best_model.pth --alpha-decay 25000000 --final-alpha 0.0 --cuda-device cuda:0


# Note that hyperparameters may originally be reported in ATARI game frames instead of agent steps
parser = argparse.ArgumentParser(description='AQT with TTQ')
...
# Config ttq loss
parser.add_argument('--ttq-target-model', type=str, default='results/rainbow/models/model.pth', help='Load ttq target model path')
parser.add_argument('--start-alpha', type=float, default=1.0, help='start alpha of ttq loss')
parser.add_argument('--final-alpha', type=float, default=0.01, help='final alpha of ttq loss')
parser.add_argument('--alpha-decay-last-frame', type=float, default=float(5e6), help='decay alpha of ttq loss ')
...
# Setup logger
logger = TTQ_REWARD_LOGGER(results_dir + "/")
...
if args.evaluate:
  ...
  for T in trange(start_step, args.T_max + 1):
    ...
    if done:
      ...
      logger.record_train_log(T, t_reward_sum, t_mean_score, metrics['t_epi'], dqn.alpha)
      log('Train: T = {0} / {1} | score: {2} | mean {3:.2f} | alpha {4:.3f}'.format(T, args.T_max, t_reward_sum, t_mean_score, dqn.alpha))
      t_reward_sum = 0.0
    ...
    if T >= args.learn_start:
      ...
      if T % args.evaluation_interval == 0:
        ...
        logger.record_eval_log(T, avg_reward, metrics['t_epi'], dqn.alpha)
      ...
      if (args.checkpoint_interval != 0) and (T % args.checkpoint_interval == 0):
        dqn.save(models_dir, 'checkpoint_{}.pth'.format(T))
